...
END subroutine Get_H_V_new
    ...
    subroutine Get_Cell_Energy(C,k)
        implicit none
            type(cel)::C(:)
           integer,intent(in)::k
           C(k)%EnergyH=Curvature_Energy( C(k)%H_modulus, C(k)%H_v, C(k)% H_zero,  C(k)%Area_V, C(k)%N_V,&
               C(K)%IS_BC_V,C(K)%IS_NEXTBC_V)
           ...
           C(k)%area_tot_zero)**2.0/C(k)%area_tot_zero
            ...
            C(k)%energy_tot=C(k)%energyp+C(k)%energyH+C(k)%energyL+C(k)%energyMs
...
! ========================================================
! Shear (MS) Energy — Skalak model
! Ported from RBC code into module form
! ========================================================
